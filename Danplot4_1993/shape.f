...
C------------------------- 3D elements ---------------------------------
      ELSEIF (NODOF.EQ.3)THEN
        ...
        ENDIF
C------------------------- 4D elements ---------------------------------
      ELSEIF (NODOF.EQ.4) THEN
        IF (NOD.EQ.16) THEN
          CALL SF16N_4D (DER,IDER,FUN,SMP, ISMP,I)
        ENDIF
      ENDIF
c-----------------------------------------------------------------------
      IF (ABS(FUN(1)-230964.).lt.0.1) 
...
C
C     This is a header to WTHATN2 to avoid unnecessary work
C
      SAVE
      REAL LCOORD  (ILCOORD,*)    !- the returned positions
     &    ,LCOORD2 (30,4)         !- saved copy (as max. possible szie)
...
c-------------- loop through the local co-ords -------------------------
      DO LEVEL=1,32700           !(ie. do-forever)
